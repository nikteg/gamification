...
export default () => (
  <div>
    <div className="Experiment-part">
		...
		<p>Graphing can be done once we have gathered our random sample. Let X be a random variable containing the distribution of the subject and x be one of the values of X. The sample distribution can then be graphed by: </p>
		<p>Y axis: frequency of X = x</p>
		<p>X axis: x</p>
		<p>Whether X is discrete or continuous, it is often necessary to group the sample units in intervals to make sure the data on the graph isn’t too spread out, especially if the sample size is small in comparison to its variance. The most common groups are 10 integer intervals (e.g the interval located at x = 0 contains all values of x from -5 to 5) and 1 integer intervals (e.g the interval located at x = 0 contains all values of x from -0.5 to 0.5). </p>
		</div>
